...
public class UserController {
	...
	public ResponseEntity<?> showAll() {
		...
		if (getUsers == null) {
			return new ResponseEntity<>(MessageResponse.builder().message("No records found").object(null).build(),
					HttpStatus.OK);
		}
		return new ResponseEntity<>(MessageResponse.builder().message("Success").object(getUsers).build(),
				HttpStatus.OK);
	}
...
//			userSave.setPassword(bCryptEncoder.encode(userDTO.getPassword()));
//			
//			return new ResponseEntity<>(MessageResponse.builder().message("Save successfully")
//					.object(userMapper.toUserDTO(userSave)).build(), HttpStatus.CREATED);
...
//	}
	...
	public ResponseEntity<?> update(@RequestBody UserDTO userDTO) {
		try {
			...
			return new ResponseEntity<>(MessageResponse.builder().message("Update successfully")
					.object(userMapper.toUserDTO(userUpdate)).build(), HttpStatus.CREATED);
		} catch (DataAccessException e) {
			return new ResponseEntity<>(MessageResponse.builder().message(e.getMessage()).object(null).build(),
					HttpStatus.METHOD_NOT_ALLOWED);
		}
	...
	public ResponseEntity<?> showById(@PathVariable Integer id) {
		...
		return new ResponseEntity<>(
				MessageResponse.builder().message("Success").object(userMapper.toUserDTO(user)).build(), HttpStatus.OK);
	}

	@GetMapping("users/role/{role}")
	public ResponseEntity<?> showAllByUserRole(
			@Parameter(description = "Role of the user: 0 = Patient, 1 = Doctor") @PathVariable Integer role) {
		UserRole userRole;

		try {
			userRole = UserRole.values()[role];
		} catch (ArrayIndexOutOfBoundsException e) {
			return new ResponseEntity<>(MessageResponse.builder().message("Invalid role").object(null).build(),
